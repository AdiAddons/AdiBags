...
local function GetBorder(bag, slot, itemId, settings)
	...
	if quality == Enum.ItemQuality.Poor and settings.dimJunk then
		local v = 1 - 0.5 * settings.qualityOpacity
		return true, v, v, v, 1, nil, nil, nil, nil, "MOD"
	end
	local color = quality ~= Enum.ItemQuality.Common and BAG_ITEM_QUALITY_COLORS[quality]
	if color then
...
function buttonProto:UpdateBorder(isolatedEvent)
	...
	if self.JunkIcon then
		...
		self.JunkIcon:SetShown(quality == Enum.ItemQuality.Poor and addon:GetInteractingWindow() == "MERCHANT")
	end
	if isolatedEvent then
