...
function containerProto:UpdateContent(bag)
	...
	for slot = 1, newSize do
		...
		if not itemId or (link and addon.IsValidItemLink(link)) then
			...
			local name, count, quality, iLevel, reqLevel, class, subclass, maxStack, equipSlot, texture, vendorPrice, classID, subclassUD, bindType, expacID, setID, isCraftingReagent
			if link then
				name, _, quality, iLevel, reqLevel, class, subclass, maxStack, equipSlot, texture, vendorPrice, classID, subclassUD, bindType, expacID, setID, isCraftingReagent = GetItemInfo(link)
				if not name then
					name, _, quality, iLevel, reqLevel, class, subclass, maxStack, equipSlot, texture, vendorPrice, classID, subclassUD, bindType, expacID, setID, isCraftingReagent = GetItemInfo(itemId)
				end
			...
			if slotData.guid ~= guid or slotData.texture ~= texture then
				...
				slotData.classID, slotData.subclassID, slotData.bindType, slotData.expacID, slotData.setID, slotData.isCraftingReagent = classID, subclassUD, bindType, expacID, setID, isCraftingReagent
