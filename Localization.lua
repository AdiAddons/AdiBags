...
L["Warning: you are using an alpha or beta version of AdiBags without displaying Lua errors. If anything goes wrong, AdiBags (or any other addon causing some error) will simply stop working for apparently no reason. Please either enable the display of Lua errors or install an error handler addon like BugSack or Swatter."] = "警告：您使用的是α或β版本的AdiBags不显示的Lua错误。如果出现任何错误，AdiBags（或任何其他插件，导致一些错误的），只是停止工作，显然没有理由。请使显示的Lua错误或安装错误处理程序插件(UI)如BugSack 或 Swatter。"
