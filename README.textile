...
* A new bag layout, which should be more readable and avoid the "section dancing" effect and works as follow, at the expense of some wasted space.
** Sections are dispatched in fixed-width columns.
** Section width is either half or the same as the column width, i.e. at most two section on the same column row.
** Sections are not updated will the bag is open: new items goes to a special section titled "recent items", which is separated from the normal sections.
...
Please report any bug or error using the "wowace bug tracker":http://www.wowace.com/addons/adibags/tickets/ (your Curse account works there). The comments section below is not meant to report and track bugs. *I won't reply to bug reports in the comment section anymore.*
...
"Bug tracker":https://github.com/Adirelle/AdiBags/issues
